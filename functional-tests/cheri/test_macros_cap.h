...
//NULL cap is root cap without tag and permissions
#define TEST_CASE_NULL_CAP(testnum, testcap, code...) \
    TEST_CASE_CAP(testnum, testcap, 0, 0x0, -1, 0x0, 0b0, -1, code)

#define TEST_CASE_ROOT_CAP_NO_OFFSET(testnum, testcap, code...) \
    TEST_CASE_CAP_NO_OFFSET(testnum, testcap, 1, 0x0, -1, 0b011110111110, -1, code)

#define TEST_CASE_ROOT_CAP(testnum, testcap, code...) \
    TEST_CASE_CAP(testnum, testcap, 1, 0x0, -1, 0x0, 0b011110111110, -1, code)
...
#define EXPECT_EXCEPTION_NONZERO_BASE(cause, capidx, code...) \
    la tp, 999f; \
    CSpecialR ct0, pcc; \
    CGetBase t6, ct0; \
    add tp, tp, t6; \
